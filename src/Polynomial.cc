...
Polynomial& Polynomial::operator=(const Polynomial& p){
  ...
  if(the_derivative!=0) delete the_derivative;
  if(the_integral!=0) delete the_integral;
  the_derivative=0;
  the_integral=0;
...
{
  return getCoefficient(num);
}

double Polynomial::getCoefficient(int num) const
{
  if(the_polynomial.size() <= num)
    return 0;
  else
    return the_polynomial[num];
}

void Polynomial::setCoefficient(int n, double val)
{
  //-- if(val==getCoefficient(n)){
    //-- return;
  //-- }

  while(the_polynomial.size()<=n){
    the_polynomial.push_back(0);
  }
  the_polynomial[n]=val;
